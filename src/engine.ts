/** chaos-cookbook — engine by zAx4hub (algo flavor: bayes) */
...
export function rank(text: string, seed = 67): number {
...
export function run(input: { items?: Item[]; threshold?: number; baseline?: string; seed?: number } = {}): Report {
  const items = input.items?.length ? input.items : [{ text: "chaos-cookbook" }];
  const threshold = input.threshold ?? 0.35;
  const seed = input.seed ?? 67;
  ...
  return {
    project: "chaos-cookbook",
    author: "zAx4hub",
    algo: "bayes",
    summary: `Processed ${findings.length} items for chaos-cookbook; avg=${avg.toFixed(3)}`,
...
export function demo(): Report {
  return run({
    items: [
      { text: "Declarative chaos for compose/k8s" },
...
export function inspect() {
  return {
    name: "chaos-cookbook",
    author: "zAx4hub",
    oneLiner: "Declarative chaos for compose/k8s",
    algo: "bayes",
    version: "0.1.0",
